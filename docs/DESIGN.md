...
每天美股开盘前一小时（**北京时间 20:30**，固定不随夏令时调整），自动抓取 54 位美股相关 Twitter / X KOL 的最新推文（含图片），用 Claude Sonnet 4.6 做 AI 总结，把当日总结推送到 GitHub 仓库主页（README.md），并按月归档历史 digest，方便公开访问。
...
对每个 active KOL（串行，间隔 sleep 2~5s）：
  ...
  情况 A：last_id IS NULL（首次接入）
    pull 50 条 → INSERT OR IGNORE → 更新 last_seen_tweet_id
...
**输出**（markdown）按 6 个内容维度分组：
1. 🔑 今日关键词（3-5 个高频主题）
2. 📰 重要新闻（政策 / 突发 / 财报 / 监管）
3. 📊 宏观判断（共识 + 分歧）
4. 🏭 产业 / 个股焦点（热议标的，含正反观点）
5. 💹 交易信号（期权异动、技术位、操作建议）
6. 💡 投资理念（值得收藏的长期视角）
...
布局：
1. 项目简介（固定）
2. 监控的 54 位 KOL 列表（一行一行 handle，链接到 X 主页；新增 KOL 时自动同步）
3. 当日总结（Layer 1 全文 + Layer 2 折叠区）
4. 历史归档索引（最近 12 个月的链接）
